...
│   ├── outliers.py              #   range rules, price clipping, IQR filtering
│   ├── features.py              #   feature engineering (ref-prefix, price history) → X, y
│   ├── training.py              #   two-stage LightGBM (select features, retrain final)
...
end to end: load → clean → remove outliers → engineer features → train a first LightGBM
model → select the features that matter (permutation importance) → **retrain the final
model** on them → evaluate. The final model scores **R² ≈ 0.96** (MAE ≈ $2,900).

It writes the data at every stage plus the final artifacts:

- `data/raw/Watches.csv` — the raw downloaded data
- `data/processed/01_cleaned.csv` — after cleaning
- `data/processed/02_no_outliers.csv` — after outlier removal
- `data/processed/03_features.csv` — the final model inputs (features + target)
- `models/model.txt` — the trained model (LightGBM booster)
- `models/metrics.json` — RMSE / MAE / R² (log + dollars) and the selected feature list
- `reports/figures/*.png` — loss curve, residuals, predicted-vs-actual, feature importance

(`data/` is git-ignored — it's regenerated by the pipeline, not committed.)
